...
# plot the datasets
dataset = "WADI"  # Change to your dataset name
data_path = f"./sub_adjacent_transformer-main/dataset/{dataset.upper()}"
...
# 创建文件夹保存生成的图像
save_dir = f"./dataset_analyze/{dataset.lower()}"
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
# Load the data
train_data = torch.from_numpy(np.load(os.path.join(data_path, train_file))).float()
test_data = torch.from_numpy(np.load(os.path.join(data_path, test_file))).float()
test_label_data = torch.from_numpy(np.load(os.path.join(data_path, test_label_file))).float()

# plot train_data
for i in range(train_data.shape[1]):
    plt.figure(figsize=(20, 6))
    plt.title(f"{dataset} Train Data - Feature {i+1}")
    plt.plot(train_data[:, i], label=f"Feature {i+1}")
    plt.xlabel("Time Steps")
    plt.ylabel("Values")
    plt.legend()
    plt.grid()
    plt.savefig(f"{save_dir}/train_feature_{i+1}.png")
    plt.close()  # 释放内存，避免大量图形对象堆积
...
for i in range(test_data.shape[1]):
    plt.figure(figsize=(20, 6))
    # 找出每段是正常（0）或异常（1）的区间
    current_label = test_label_data[0]
    start_idx = 0

    for idx in range(1, len(test_label_data)):
        if test_label_data[idx] != current_label:
            segment = test_data[start_idx:idx, i]
            color = 'red' if current_label == 1 else 'royalblue'
            plt.plot(np.arange(start_idx, idx), segment, color=color, label='Anomaly' if current_label == 1 else 'Normal')
            start_idx = idx
            current_label = test_label_data[idx]

    # 画最后一段
    segment = test_data[start_idx:, i]
    color = 'red' if current_label == 1 else 'royalblue'
    plt.plot(np.arange(start_idx, len(test_label_data)), segment, color=color, label='Anomaly' if current_label == 1 else 'Normal')
    # 设置图标题和标签
    plt.title(f"{dataset} Test Data - Feature {i+1}")
    plt.xlabel("Time Steps")
    plt.ylabel("Values")
    # 辅助线与图例
    plt.grid(True)
    plt.legend(['Normal', 'Anomaly'])  # 简化图例
    plt.tight_layout()
    # 保存图像
    plt.savefig(f"{save_dir}/test_feature_{i+1}.png")
    plt.close()  # 释放内存，避免大量图形对象堆积
