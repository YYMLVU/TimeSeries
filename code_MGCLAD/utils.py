...
def create_data_loaders(train_dataset, batch_size, val_split=0.1, shuffle=True, test_dataset=None, train_aug_dataset=None):
    train_loader, val_loader, test_loader, train_aug_loader = None, None, None, None
    if val_split == 0.0:
        print(f"train_size: {len(train_dataset)}")
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
        if train_aug_dataset is not None:
            train_aug_loader = torch.utils.data.DataLoader(train_aug_dataset, batch_size=batch_size, shuffle=shuffle)
    ...
    return train_loader, train_aug_loader, val_loader, test_loader
...
def plot_losses_mem1(losses, save_path="", plot=True):
    """
    :param losses: dict with losses
    :param save_path: path where plots get saved
    """
    plt.plot(losses["train_dist"], label="dist loss")
    plt.plot(losses["train_inter"], label="inter loss")
    plt.plot(losses["train_recon"], label="Recon loss")
    plt.plot(losses["train_total"], label="Total loss")
    plt.title("Training losses during training")
    plt.xlabel("Epoch")
    ...
    plt.plot(losses["val_dist"], label="Forecast loss")
    plt.plot(losses["val_inter"], label="Forecast loss")
    plt.plot(losses["val_recon"], label="Recon loss")
    plt.plot(losses["val_total"], label="Total loss")
    plt.title("Validation losses during training")
    plt.xlabel("Epoch")
...
def plot_losses_recon(losses, save_path="", plot=True):
    """
    :param losses: dict with losses
    :param save_path: path where plots get saved
    """
...
def plot_losses_mem(losses, save_path="", plot=True):
    """
    :param losses: dict with losses
    :param save_path: path where plots get saved
    """
    ...
    plt.plot(losses["train_recon"], label="Recon loss")
    plt.plot(losses["train_mem"], label="Mem loss")
    plt.plot(losses["train_total"], label="Total loss")
    plt.title("Training losses during training")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    ...
    plt.plot(losses["val_recon"], label="Recon loss")
    plt.plot(losses["val_mem"], label="Mem loss")
    plt.plot(losses["val_total"], label="Total loss")
    plt.title("Validation losses during training")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
...
def plot_losses_nf(losses, save_path="", plot=True):
    """
    :param losses: dict with losses
    :param save_path: path where plots get saved
    """

    plt.plot(losses["train_nf"], label="nf loss")
    plt.plot(losses["train_total"], label="Total loss")
    plt.title("Training losses during training")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    ...
    plt.plot(losses["val_nf"], label="nf loss")
    plt.plot(losses["val_total"], label="Total loss")
    plt.title("Validation losses during training")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
...
def adjust_anomaly_scores(scores, dataset, is_train, lookback):
    ...
    if is_train:
        md = pd.read_csv(f'./code_MGCLAD/datasets/data/{dataset.lower()}_train_md.csv')
    else:
        md = pd.read_csv('./code_MGCLAD/datasets/data/labeled_anomalies.csv')
